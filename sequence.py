...
def stap_length(row,col,idx=0):
    ...
    if row_new == -1 and col_new == -1:
        ...
        idx +=1
        return stap_length(find_num(row_new),col_new,idx)
...
def fill_scaf(stap_row,stap_col,stap_seq,idx=0):
...
for i, strand in enumerate(a["vstrands"]):
    for j, col in enumerate(strand["stap_colors"]):
        L = stap_length(i,col[0])
        strand["stap_seq"] +=[[col[0], ''.join([B.keys()[x] for x in np.random.random_integers(0,3,L)])]]
        fill_scaf(i,col[0],strand["stap_seq"][-1][-1])
    #print ''.join(strand["scaf_seq"]),strand["stap_seq"][-1][-1]
